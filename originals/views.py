...
def IOW_challenge03(request):
	thisEvent = get_object_or_404(InOtherWord, challengeNo=3)
	if thisEvent.active:
		if request.user.is_authenticated:
			allResponses = InOtherWordsChallenge03.objects.all()
			allUserData = UserData.objects.all()
			isResponded = False
			thisInstance = False
			thisUserData = False
			...
			if isResponded:
				if thisInstance.isSubmit:
					...
					f = InOtherWordsChallengeForm03(instance=thisInstance)
					if request.method == "POST":
						#request.FILES if needed
						f = InOtherWordsChallengeForm03(request.POST, instance=thisInstance)
						if f.is_valid():
							thisInstance = f.save(commit=False)
							if request.POST.get("submit"):
								thisInstance.isSubmit = True
								...
								messages.add_message(request, messages.INFO, 'You have succesfully modified your  \'In Other Words\' Challenge \' ' + (thisEvent.title) + ' \' Responses')
								f =InOtherWordsChallengeForm03(instance=thisInstance)
								return render(request, 'originals/iow_challenge03.html', {'form': f})
			else:
				if request.method == "POST":
					#request.FILES if needed
					f = InOtherWordsChallengeForm03(request.POST)
					if f.is_valid():
						reg = f.save(commit=False)
						reg.user = request.user
						if request.POST.get("submit"):
...
def IOW_challenge04(request):
	thisEvent = get_object_or_404(InOtherWord, challengeNo=4)
	if thisEvent.active:
		if request.user.is_authenticated:
			allResponses = InOtherWordsChallenge04.objects.all()
			allUserData = UserData.objects.all()
			isResponded = False
			thisInstance = False
			thisUserData = False
			...
			if isResponded:
				if thisInstance.isSubmit:
					...
					f = InOtherWordsChallengeForm04(instance=thisInstance)
					if request.method == "POST":
						#request.FILES if needed
						f = InOtherWordsChallengeForm04(request.POST, instance=thisInstance)
						if f.is_valid():
							thisInstance = f.save(commit=False)
							if request.POST.get("submit"):
								thisInstance.isSubmit = True
								...
								messages.add_message(request, messages.INFO, 'You have succesfully modified your  \'In Other Words\' Challenge \' ' + (thisEvent.title) + ' \' Responses')
								f =InOtherWordsChallengeForm04(instance=thisInstance)
								return render(request, 'originals/iow_challenge04.html', {'form': f})
			else:
				if request.method == "POST":
					#request.FILES if needed
					f = InOtherWordsChallengeForm04(request.POST)
					if f.is_valid():
						reg = f.save(commit=False)
						reg.user = request.user
						if request.POST.get("submit"):
...
def IOW_challenge06(request):
	thisEvent = get_object_or_404(InOtherWord, challengeNo=6)
	if thisEvent.active:
		if request.user.is_authenticated:
			allResponses = InOtherWordsChallenge06.objects.all()
			allUserData = UserData.objects.all()
			isResponded = False
			thisInstance = False
			thisUserData = False
			...
			if isResponded:
				if thisInstance.isSubmit:
					...
					f = InOtherWordsChallengeForm06(instance=thisInstance)
					if request.method == "POST":
						#request.FILES if needed
						f = InOtherWordsChallengeForm06(request.POST, instance=thisInstance)
						if f.is_valid():
							thisInstance = f.save(commit=False)
							if request.POST.get("submit"):
								thisInstance.isSubmit = True
								...
								messages.add_message(request, messages.INFO, 'You have succesfully modified your  \'In Other Words\' Challenge \' ' + (thisEvent.title) + ' \' Responses')
								f =InOtherWordsChallengeForm06(instance=thisInstance)
								return render(request, 'originals/iow_challenge06.html', {'form': f})
			else:
				if request.method == "POST":
					#request.FILES if needed
					f = InOtherWordsChallengeForm06(request.POST)
					if f.is_valid():
						reg = f.save(commit=False)
						reg.user = request.user
						if request.POST.get("submit"):
